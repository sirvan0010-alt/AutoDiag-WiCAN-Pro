...
/** Backward-compatible entry point used by the current connection screen. */
@Composable
fun OutlanderPhevResistanceCard(
    ...
    OutlanderPhevResistanceCard(isolation, maxStats, minStats)
}
...
fun OutlanderPhevResistanceCard(
    ...
    val stats = mapOf(OutlanderResistanceKind.HV_ISOLATION_RESISTANCE to isolation, OutlanderResistanceKind.INTERNAL_RESISTANCE_MAX_UNVERIFIED to internalMax, OutlanderResistanceKind.INTERNAL_RESISTANCE_MIN_UNVERIFIED to internalMin)
    ...
    LaunchedEffect(isolation.current, internalMax.current, internalMin.current, samplingMs) {
        ...
        if (now - lastRecordedMs >= samplingMs) {
            ...
            stats.forEach { (kind, value) -> value.current?.let { current -> updated[kind] = (updated[kind].orEmpty() + OutlanderResistanceSample(now, current, value.verification)).takeLast(2_000) } }
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            Text("Kliknutím na hodnotu otevřete živý graf. Průběh se během měření průběžně doplňuje.", style = MaterialTheme.typography.bodySmall)
            Text("Vzorkování grafu: ${samplingMs} ms", style = MaterialTheme.typography.labelMedium)
            Row(Modifier.horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(6.dp)) { OutlanderLiveSamplingSettings.OPTIONS_MS.forEach { option -> OutlinedButton(onClick = { samplingMs = option }) { Text("${option} ms") } } }
            Text("Nastavení určuje minimální interval ukládání vzorků do grafu. Nezvyšuje samo o sobě rychlost ECU; skutečnou frekvenci určuje diagnostický request/response cyklus.", style = MaterialTheme.typography.bodySmall)
            Text("DŮLEŽITÉ: MAX/MIN pochází ze zdroje, který je nazývá internal resistance, ale nevíme, co fyzicky znamenají. Nejsou prezentovány jako potvrzený vnitřní odpor (ESR) trakční baterie.", style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.error)
    ...
    graphKind?.let { kind ->
        val samples = localSamples[kind].orEmpty()
        AlertDialog(onDismissRequest = { graphKind = null }, confirmButton = { OutlinedButton(onClick = { graphKind = null }) { Text("Zavřít") } }, title = { Text(kind.displayNameCs) }, text = {
            Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                ...
                ResistanceLiveGraph(samples, kind.unit)
