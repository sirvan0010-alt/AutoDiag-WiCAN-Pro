...
import com.autodiag.wican.ui.components.OutlanderResistanceCard
        ...
        setContent {
            AutoDiagTheme {
                ...
                if (conn.phase == ConnectionPhase.IDLE) {
                    DiscoveryScreen(discovery, connectionViewModel::connectElm327, connectionViewModel::connectSlcan, connectionViewModel::connectSimulator)
                } else {
                    ConnectionResultScreen(conn, connectionViewModel::disconnect, {
                        when (conn.mode) {
            ...
            when (state.phase) {
                ...
                ConnectionPhase.READY -> {
                    if (state.linkOnly && state.mode == TransportMode.SLCAN_RAW) {
                        RawCanMonitorScreen(state = state.rawCanMonitor, onFilterChanged = onRawCanFilter, onPauseToggle = onRawCanPause, onClear = onRawCanClear, onDisconnect = onDisconnect)
                    } else {
                        ...
                        OutlanderResistanceCard(state.outlanderIsolation, state.outlanderInternalResistance, expertRawRequest = state.outlanderExpertRequest, expertRawResponse = state.outlanderExpertResponse, showExpert = expert)
                        OutlinedButton(onClick = { expert = !expert }, Modifier.fillMaxWidth()) { Text(if (expert) "Skrýt Expert raw" else "Zobrazit Expert raw") }
                        Spacer(Modifier.height(8.dp))
