        ...
        LazyColumn(
            ...
            item {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    ...
                    InfoTooltip("AutoDiag nejprve hledá WiCAN v místní síti pomocí mDNS. Nalezený endpoint ještě neznamená podporu diagnostických funkcí.")
                }
            }
            ...
            item {
                WiCanProtocolCard(onSelect = { mode ->
                    val device = devices.firstOrNull()
                    when (mode) {
                        TransportMode.ELM327 -> if (device != null) { val e = device.suggestedElm327Endpoint(); onConnectElm(e.first, e.second) } else if (manualIp.isNotBlank()) onConnectElm(manualIp.trim(), 3333)
                        TransportMode.SLCAN_RAW -> if (device != null) { val s = device.suggestedSlcanEndpoint(); onConnectSlcan(s.first, s.second) } else if (manualIp.isNotBlank()) onConnectSlcan(manualIp.trim(), 23)
                ...
                item {
                    Card(Modifier.fillMaxWidth()) {
                        Column(Modifier.padding(16.dp)) {
                            ...
                            OutlinedTextField(manualIp, { manualIp = it }, label = { Text("IP adresa WiCAN") }, modifier = Modifier.fillMaxWidth())
                            Spacer(Modifier.height(8.dp))
                            Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
            ...
            item {
                Card(Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp)) {
                        ...
                        Spacer(Modifier.height(12.dp))
                        Button(onClick = onConnectSimulator, modifier = Modifier.fillMaxWidth()) { Text("Připojit simulátor") }
                    }
                }
            }
        }
...
private fun VinAuditCard(audit: VinAudit) {
    ...
    Card(Modifier.fillMaxWidth()) {
        Column(Modifier.padding(12.dp)) {
            Text("Kontrola VIN v řídicích jednotkách", fontWeight = FontWeight.Medium)
            ...
            if (audit.hasMismatch) {
                ...
                audit.ecuRecords.forEach { record ->
                    val mismatch = audit.referenceVin != null && record.vin != audit.referenceVin
                    Text("${record.displayEcu}: ${record.vin}${if (mismatch) "  ← ODLIŠNÉ VIN" else ""}", color = if (mismatch) MaterialTheme.colorScheme.error else MaterialTheme.colorScheme.onSurface)
                }
            } else {
