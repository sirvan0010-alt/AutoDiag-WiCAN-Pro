...
object OutlanderPhevResistanceDecoder {
    ...
    /** Watchdog e4/a, command 21 01: byte 38, MΩ. */
    fun decodeMaximumInternalResistance(response: IntArray): Double {
        require(response.size > 38) { "Outlander battery response is too short" }
        return (response[38] and 0xFF) / 10.0
    }

    /** Watchdog e4/a, command 21 01: byte 39, MΩ. */
    fun decodeMinimumInternalResistance(response: IntArray): Double {
