...
object DiagnosticCatalogParser {
    ...
    fun recordCounts(manifestBody: String): Map<String, Int> {
        ...
        return mapOf("vehicles" to r.optInt("vehicles", 0), "ecus" to r.optInt("ecus", 0), "signals" to r.optInt("signals", 0), "dtc" to maxOf(r.optInt("dtc", 0), r.optInt("dtcs", 0)))
    }
    ...
    private fun provenance(o: JSONObject): String {
        if (o.has("provenance") && o.get("provenance") is JSONObject) return o.getJSONObject("provenance").optString("source_id", "diagnostic-data")
