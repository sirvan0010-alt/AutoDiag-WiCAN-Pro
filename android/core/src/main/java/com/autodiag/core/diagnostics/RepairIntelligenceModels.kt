...
enum class RepairProcedureType {
    DIAGNOSIS,
    MECHANICAL_REPAIR,
    ELECTRICAL_REPAIR,
    WIRING,
...
data class RepairPart(
    ...
    val newPartRequired: Boolean? = null,
...
data class LaborEstimate(
    ...
    val hours: Double,
    val timeType: LaborTimeType,
    val sourceId: String? = null,
    val correctionCode: String? = null,
...
data class PriceEstimate(
    ...
    init {
        require(min >= 0.0) { "Minimum price cannot be negative" }
        require(max >= min) { "Maximum price cannot be below minimum price" }
    }
}
...
data class RepairProcedure(
    ...
    val torqueNotes: List<String> = emptyList(),
    val postRepairChecks: List<String> = emptyList(),
    val estimatedLabor: LaborEstimate? = null,
    val verification: VerificationState = VerificationState.UNVERIFIED
)

data class RepairEstimate(
    val parts: List<RepairPart> = emptyList(),
    val labor: List<LaborEstimate> = emptyList(),
    val currency: String = "EUR",
    val totalMin: Double,
    val totalMax: Double
) {
    init {
        require(totalMin >= 0.0) { "Minimum total cannot be negative" }
        require(totalMax >= totalMin) { "Maximum total cannot be below minimum total" }
    }
}
...
data class RepairIntelligence(
    val dtcCode: String,
    val ecu: String? = null,
    val vehicleScope: VehicleScope = VehicleScope(),
    val meaning: String? = null,
    val severity: String? = null,
    val possibleCauses: List<String> = emptyList(),
    val diagnosticChecks: List<String> = emptyList(),
    val candidateParts: List<RepairPart> = emptyList(),
    val procedures: List<RepairProcedure> = emptyList(),
    val estimate: RepairEstimate? = null,
    val sources: List<RepairSource> = emptyList(),
    val verification: VerificationState = VerificationState.UNVERIFIED
)

/** Prevents a UI shortcut from turning a DTC into a deterministic part failure. */
fun RepairIntelligence.hasDeterministicPartVerdict(): Boolean =
    candidateParts.size == 1 &&
        candidateParts.single().confidence == VerificationState.VERIFIED &&
