...
object RepairEstimateEngine {
    fun calculate(parts: List<RepairPart>, labor: List<LaborEstimate>, currency: String = "EUR", fallbackHourlyRate: Double? = null): RepairEstimate? {
        val partTotals = parts.map { part ->
            val price = part.price?.takeIf { it >= 0.0 } ?: return null
            price * part.quantity
        }
        val laborTotals = labor.map { item ->
            val rate = item.hourlyRate?.takeIf { it >= 0.0 } ?: fallbackHourlyRate?.takeIf { it >= 0.0 } ?: return null
            rate * item.hours
        }
        val total = partTotals.sum() + laborTotals.sum()
        return RepairEstimate(parts = parts, labor = labor, currency = currency, totalMin = total, totalMax = total)
    }
}
