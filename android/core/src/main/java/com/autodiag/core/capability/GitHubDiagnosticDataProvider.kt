...
class GitHubDiagnosticDataProvider(
    ...
    override suspend fun findDecoderCandidates(request: String, variantId: String?): List<SignalDecoderDefinition> = withContext(Dispatchers.IO) {
        ...
        decoderCandidates.filter { it.request.equals(normalized, true) && (variantId == null || it.variantId.equals(variantId, true)) }
    }
            ...
            val candidateFiles = manifest.optJSONArray("candidateFiles")
                ...
                ?: throw IllegalStateException(
                    "manifest.json declares records.candidates=$declaredCandidateCount but has no " +
                        "\"candidateFiles\" or \"canonicalCandidateFiles\" array. The candidate file list must be data-driven from manifest.json."
                )
...
class UrlConnectionDiagnosticDataHttpClient(private val connectTimeoutMs: Int = 5000, private val readTimeoutMs: Int = 10000) : DiagnosticDataHttpClient {
    override fun get(url: String): String {
        ...
        try { c.requestMethod = "GET"; c.connectTimeout = connectTimeoutMs; c.readTimeout = readTimeoutMs; c.setRequestProperty("Accept", "application/json"); if (c.responseCode !in 200..299) throw IllegalStateException("Diagnostic data HTTP ${c.responseCode}"); return BufferedReader(InputStreamReader(c.inputStream, Charsets.UTF_8)).use { it.readText() } }
