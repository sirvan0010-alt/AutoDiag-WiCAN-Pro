...
## Open development, independent re-implementation

This project does **not** ship or embed proprietary diagnostic binaries, encrypted label databases (CLB/LBL/XPL), protected coding databases, credentials or manufacturer-internal security material.

For VAG, Mitsubishi PHEV and other manufacturers the goal is **functional interoperability**. Direct binary copying is forbidden. Independent reverse-engineering, live-bus observation, public documentation, licensed data and verified engineering work **are allowed and expected**.

Signal maps, PID lists, coding tables and thresholds derived from competitor applications (PHEV Watchdog, Torque, Car Scanner, Mitsubishi Remote Ctrl, LCode, etc.) or from real-vehicle captures may be re-implemented when they carry explicit provenance and verification status.

The project can therefore pursue measuring values, basic settings, adaptations, service functions, gateway/topology and carefully controlled coding/long-coding workflows once the protocol, data, safety, legal basis and vehicle scope are verified.

Value comes from architecture, engineering, testing, evidence and community — not from redistributing commercial binaries.
